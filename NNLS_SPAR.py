...
for example; Variance will be negative")
    ...
    def qr_lstsq(self,ind):
        ...
        qs,rs = linalg.qr(self.rtilde[:t,ind])
    ...
    def qr_lstsql(self,ind):
        ...
        rhs = transpose(qs).dot(self.qb[:t,[0]])
        x = solve_triangular(rs[:l,:l],rhs[:l,0])
        res = norm(self.rtilde[:t,ind].dot(x)-self.qb[:t,0]) ** 2 + norm(self.qb[t:]) ** 2 + self.permanentresidual
        """ using QR to solve a least squares problem """
            ...
            while L[i].qsize() >= 1:
                ...
                if len_c < self.s:
                    if  count_nonzero(coef) <= self.s:
                        ...
                        self.indexes.append(where(coef > 0)[0])
                        self.coefficients.append(coef[where(coef > 0)[0]])
                        count_best += 1
    ...
    def solve_nnls(self,P ,C = []):
        ...
        while q.qsize() > 0:
            ...
            if len_c < self.s:
                if  count_nonzero(coef) <= self.s:
                    self.residual_squared.append(low)
                    self.indexes.append(list(where(coef > 0)[0]))
                    self.coefficients.append(coef[where(coef > 0)[0]])
                    count_best += 1
                    if count_best == self.many:
